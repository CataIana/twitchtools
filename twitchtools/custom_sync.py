...
from disnake import Client, ApplicationCommand
...
def _show_diff(self, diff: Dict[str, List[ApplicationCommand]]) -> None:
    ...
    change_type = f", ".join(cmd.name for cmd in diff["change_type"]) or None
    ...
    if change_type:
        self.log.info(f"Application Commands To Change Type: {change_type}")
    if not to_upsert and not to_edit and not to_delete and not change_type:
...
async def _sync_application_commands(self) -> None:
    ...
    if update_required:
        ...
        try:
            ...
            self.log.error(f"Global command override failed due to {e}")
    ...
    for guild_id, cmds in guild_cmds.items():
        ...
        if update_required:
            try:
                ...
                self.log.error(f"Command overwrite failed for {self.get_guild(int(guild_id))} due to {e}")
    # Last debug message
    if self._sync_commands_debug:
