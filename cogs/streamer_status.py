...
from twitchtools.enums import ChannelCache
...
class StreamStatus(commands.Cog):
    ...
    async def on_title_change(self, event: TitleEvent):
        ...
        if updated == []:  # If for some reason neither the title or game updated, just ignore
            self.bot.log.info(
                f"No title updates for {event.broadcaster.username}, ignoring")
            return
        ...
        if stream:
            ...
            if self.ratelimits.get(str(event.broadcaster.id), None) is None:
                self.ratelimits[str(event.broadcaster.id)] = Ratelimit(
                    calls=10, period=600)
            try:
                self.ratelimits[str(event.broadcaster.id)].request()
            except RateLimitExceeded:
                self.bot.log.info(
                    f"Title update ratelimit for {event.broadcaster.username} exceeded!")
                return
        ...
        embed = disnake.Embed(
            url=f"https://twitch.tv/{event.broadcaster.username}", colour=0x812BDC, timestamp=utcnow())
        if event.title != old_title:
        ...
        self.bot.log.info(
            f"Sending title update for {event.broadcaster.username}")
    ...
    async def on_streamer_offline(self, streamer: Union[User, PartialUser]):
        ...
        if channel_cache.get("stream_id", None):
            vod = await self.bot.api.get_video_from_stream_id(streamer, channel_cache.stream_id)
                ...
                try:  # Try to get the live alerts message, skipping if not found
                    ...
                    try:  # Get the live alert embed, skipping it was removed, or something else happened
                        ...
                        try:  # Replace the applicable strings with past tense phrasing
                            ...
                            self.bot.log.warning(
                                f"Error editing message to offline in {channel.guild.name}")
    ...
    def is_live(self, channel_cache: ChannelCache):
    ...
    async def on_streamer_online(self, stream: Stream):
        ...
        if self.is_live(channel_cache):
            if stream.origin == AlertOrigin.callback:
                self.bot.log.info(
                    f"Ignoring alert while live for {stream.user.username}")
            return

        if on_cooldown:  # There is a 10 minute cooldown between alerts, but live channels will still be created
            self.bot.log.info(
                f"Cooldown active for {stream.user.username}, updating/recreating channels and restoring notification messages")

        self.bot.log.info(
            f"Updating status to online for {stream.user.username}")
        ...
        for guild_id, alert_info in callback["alert_roles"].items():
            ...
            if alert_info["mode"] == 0:  # Temporary live channel mode
                ...
                try:
                    ...
                    self.bot.log.warning(
                        f"Permission error creating text channels in guild {guild.name}! ({stream.user.username})")
            ...
            # Permanent channel. Do the same as above, but modify the existing channel, instead of making a new one
                ...
                if channel is not None:
                    try:
                        ...
                        self.bot.log.warning(
                            f"Forbidden error updating {stream.user.username} in guild {channel.guild.name}")
                else:
                    self.bot.log.warning(
                        f"Error fetching channel ID {alert_info['channel_id']} for {stream.user.username}1")
